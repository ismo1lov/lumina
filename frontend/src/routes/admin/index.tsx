...
interface AdminOrder {
  ...
  subtotal: number;
  total: number;
  shippingAddress: { name: string; phone: string; address: string; city: string; lat?: string; lng?: string } | null;
  payment: string;
  delivery: string;
  createdAt: string;
  userId: string;
...
function AdminPage() {
  ...
  const loadOrders = useCallback(() => {
    api.get<AdminOrder[]>("/admin/orders").then(setOrders).catch(() => {});
  }, []);
  ...
  const loadContacts = useCallback(() => {
    api.get<Contact[]>("/admin/contacts").then(setContacts).catch(() => {});
  }, []);
  ...
  const sendReply = async () => {
    if (!replyContact || replyText.trim().length < 1) return;
    setReplying(true);
    setReplyOk("");
    try {
      await api.post(`/admin/contacts/${replyContact.id}/reply`, { message: replyText.trim() });
      setReplyOk("Reply sent to the user");
      setReplyText("");
      setTimeout(() => {
        setReplyOk("");
              ...
              {tab === "overview" && (
                <div>
                  <div className="grid grid-cols-2 gap-4 lg:grid-cols-5">
                    <StatCard delay={0.05} icon={Users} label="Total users" value={Number(stats?.users ?? 0)} />
                    <StatCard delay={0.12} icon={ShoppingBag} label="Orders" value={Number(stats?.orders ?? 0)} />
                    <StatCard delay={0.19} icon={Wallet} label="Revenue" value={Number(stats?.revenue ?? 0)} suffix=" UZS" />
                    <StatCard delay={0.26} icon={Clock} label="Pending" value={Number(stats?.pendingOrders ?? 0)} />
                    <StatCard delay={0.33} icon={MessageSquare} label="Messages" value={Number(stats?.contacts ?? 0)} />
                  </div>
                  ...
                  {orders.length === 0 ? (
                    ...
                    orders.map((o, i) => (
                      <motion.div
                        ...
                        initial={{ opacity: 0, y: 14 }}
                        animate={{ opacity: 1, y: 0 }}
                        transition={{ delay: Math.min(0.05 * i, 0.3), duration: 0.4, ease: "easeOut" }}
                      ...
                      >
                        ...
                        <div className="grid gap-x-8 gap-y-4 px-5 py-4 sm:grid-cols-2">
                          ...
                          <div className="sm:col-span-2">
                            <p className="text-[10px] uppercase tracking-[0.18em] text-muted-foreground">Delivery address</p>
                          ...
                          <div>
                            ...
                            <p className="mt-1 text-sm capitalize">{o.payment ? (o.payment === "cod" ? "Cash on delivery" : o.payment) : "—"}</p>
                          </div>
                          <div>
                            <p className="text-[10px] uppercase tracking-[0.18em] text-muted-foreground">Delivery method</p>
                            <p className="mt-1 text-sm">{o.delivery ? (o.delivery === "express" ? "Express + assembly" : "Standard") : "—"}</p>
                          </div>
                        </div>
                            ...
                            <div className="mt-2 divide-y divide-border/60">
                              {o.items.map((it) => (
                                <div key={it.id} className="flex items-center justify-between gap-3 py-2 text-sm">
                                  ...
                                  <span className="whitespace-nowrap">{fmt(it.unitPrice * it.qty)} UZS</span>
                                </div>
                              ))}
                            </div>
                          ...
                          <div className="flex shrink-0 items-center gap-1">
                            {c.userId && (
                              <motion.button
                                ...
                                title="Reply to user"
                                className="cursor-pointer rounded-full p-2 text-muted-foreground/50 transition-all hover:bg-walnut/10 hover:text-walnut sm:opacity-0 sm:group-hover:opacity-100"
                              >
                                <Reply size={15} />
                              </motion.button>
                            )}
                          ...
                          {detail.orders.map((o) => (
                            ...
                            >
                              <div className="flex items-center justify-between">
                                ...
                                <div className="flex items-center gap-3">
                                  <StatusBadge status={o.status} />
                                  <span className="font-semibold tabular-nums text-sm">{fmt(o.total)} UZS</span>
                                </div>
                              </div>
                              {o.items.length > 0 && (
          ...
          {replyContact && (
            ...
            >
              <motion.div
                ...
                transition={{ duration: 0.32, ease: [0.22, 1, 0.36, 1] }}
                onClick={(e) => e.stopPropagation()}
                className="w-full max-w-lg rounded-3xl bg-white p-6 shadow-[0_40px_90px_-30px_rgba(38,27,20,0.5)] sm:p-8"
              >
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <h3 className="font-display text-xl">Reply to {replyContact.name}</h3>
                    {replyContact.userName && (
                      <p className="mt-1 flex items-center gap-1.5 text-xs text-muted-foreground">
                        <Users size={12} />
                        {replyContact.userName} · {replyContact.userEmail}
                      </p>
                    )}
                  </div>
                  <button
                ...
                <div className="mt-5 rounded-2xl border border-border bg-background/60 p-4">
                  <p className="text-[10px] uppercase tracking-[0.15em] text-muted-foreground">{replyContact.subject}</p>
                  <p className="mt-2 text-sm leading-relaxed text-muted-foreground">{replyContact.message}</p>
                </div>
                ...
                <textarea
                  ...
                  className="mt-5 w-full resize-none rounded-2xl border border-border bg-background/60 p-4 text-sm outline-none focus:border-walnut/40"
                />

                {replyOk ? (
                  <p className="mt-4 text-center text-sm text-emerald-600">{replyOk}</p>
                ) : (
                  <button
                    type="button"
                    onClick={sendReply}
                    disabled={replying || replyText.trim().length < 1}
                    className="mt-5 flex w-full cursor-pointer items-center justify-center gap-2 rounded-2xl bg-walnut py-3 text-[11px] uppercase tracking-[0.2em] text-cream transition-opacity hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-40"
                  >
                    <Send size={13} />
